...
private slots:
    void opensStagesAndReadsStatus()
    {
        ...
        QCOMPARE(git_repository_init(&repo,dir.path().toUtf8().constData(),0),0);
        ...
        auto state=backend.status(&error); QCOMPARE(state.files.size(),1);
        ...
        state=backend.status(&error); QVERIFY(state.files[0].isStaged());
    }
};
