...
| 配置 | 结果 |
| --- | --- |
| MSVC 2019 x64 + 预编译 OpenSSL 3.0.16 + 内置 libgit2 | 全新目录构建通过，CTest 6/6 通过，GitHub HTTPS 实际克隆通过；OpenSSL DLL 和 CA 包自动部署 |
...
测试直接使用临时仓库和内置 libgit2，不调用系统 Git。当前覆盖 Diff hunk 解析、状态读取、中文/空格/特殊字符路径、无尾随换行、提交与分支、超过 1000 条提交的分页与查询过滤、异步查询、取消后补偿刷新和仓库切换隔离、重命名暂存、补丁正反向应用、Stash、丢弃、Abort 安全边界、Push upstream 目标，以及 Merge、普通 Rebase、Cherry-pick、Revert、三种 Reset 和六种 Interactive Rebase 动作。历史操作测试还覆盖 merge commit 的两个 mainline、冲突 Continue/Abort、edit 暂停、会话绑定、关闭重开、写前恢复、rewritten 映射备份、Git notes 迁移、外部多提交 sequencer 安全拒绝、分支身份漂移防护、快进引用锁冲突恢复，以及危险状态下的分支/标签写入门禁。
