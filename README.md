...
| 配置 | 结果 |
| --- | --- |
| MSVC 2019 x64 + 预编译 OpenSSL 3.0.16 + 内置 libgit2 | 全新目录构建通过，CTest 5/5 通过，GitHub HTTPS 实际克隆通过；OpenSSL DLL 和 CA 包自动部署 |
...
3. 在中间提交图中筛选或继续加载历史；选择提交可查看差异，右键可复制 Hash、查看父子提交或管理标签。
...
测试直接使用临时仓库和内置 libgit2，不调用系统 Git。当前覆盖 Diff hunk 解析、状态读取、中文/空格/特殊字符路径、无尾随换行、提交与分支、超过 1000 条提交的分页与查询过滤、异步查询和仓库切换隔离、重命名暂存、补丁正反向应用、Stash、丢弃、Abort 安全边界和 Push upstream 目标。
