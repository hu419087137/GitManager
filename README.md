...
- Perl 和平台构建工具，用于从仓库内源码构建 OpenSSL 3.0.16

在 Windows 上可使用 Qt Online Installer 安装 Qt 6、CMake 和 Ninja，并选择与 Qt 套件匹配的 MinGW 或 MSVC 编译器。

项目将 `third_party/libgit2-1.9.4` 和 `third_party/OpenSSLv3.0.16` 直接作为源码子项目构建，不依赖系统预装的 libgit2/OpenSSL。Windows 需要 Perl 与 `nmake`，Ubuntu 需要 Perl、Make 和基础编译工具，macOS 需要 Perl、Make 和 Xcode Command Line Tools。
